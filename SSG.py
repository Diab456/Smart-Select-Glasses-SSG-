...
class Dot(object):
    ...
    def draw(self,canvas):
        ...
        #canvas.creates_text(x,y,text=str(self.n))
...
def drawBackground(canvas):
    ...
    data.backgroundColor=rgbString(22,18,27)
...
def doDotDrawings(canvas):
    ...
    for dot in data.dots:
        dot.draw(canvas)
    for i in range(1,len(data.dots)):
...
def giveGlassesDisplayObjectsPositionsAndPageNumbers(glassesList):
    ...
    for i in range(1,data.totalPages+1):
        for j in range(1,data.numSpots+1):
...
def initTryOnData():
    ...
    width = int(data.cap.get(cv2.CAP_PROP_FRAME_WIDTH))
    height = int(data.cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
    data.frameCounter=0
...
def run():
    ...
    root.wm_title("Smart Select Glasses")
    global data 
    data=Struct()
    resetData() 
    storePhaseBooleans() #tells us the phase in the program we're at
